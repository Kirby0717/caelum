...
pub fn distribute(constraints: &[(f64, RangeInclusive<u16>)], total: u16) -> Vec<u16> {
    assert!(
        constraints
            .iter()
    ...
    let sum_lo = constraints.iter().map(|(_, r)| r.start as u32).sum::<u32>();
    let sum_hi = constraints.iter().map(|(_, r)| r.last as u32).sum::<u32>();
    let t = total as u32;

    if t < sum_lo {
        let lo: Vec<f64> = constraints.iter().map(|(_, r)| r.start as f64).collect();
        weighted_round(total, &lo)
    } else if t > sum_hi {
        let hi: Vec<f64> = constraints.iter().map(|(_, r)| r.last as f64).collect();
...
fn distribute_by_waterlevel(constraints: &[(f64, RangeInclusive<u16>)], total: u16) -> Vec<u16> {
    let h = find_waterlevel(constraints, total as f64);
    let ideals: Vec<f64> = constraints
        .iter()
        .map(|(w, r)| (w * h).clamp(r.start as f64, r.last as f64))
        .collect();
    round_largest_remainder(&ideals, total)
}

fn find_waterlevel(constraints: &[(f64, RangeInclusive<u16>)], target: f64) -> f64 {
    let f_at = |h: f64| -> f64 {
        constraints
            .iter()
            .map(|(w, r)| (w * h).clamp(r.start as f64, r.last as f64))
